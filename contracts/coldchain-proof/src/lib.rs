...
#[contract]
pub struct ColdchainProofContract;

#[contractimpl]
impl ColdchainProofContract {
...
mod test {
    ...
    fn test_initialize() {
        let env = Env::default();
        env.mock_all_auths();
        let contract_id = env.register_contract(None, ColdchainProofContract);
        let client = ColdchainProofContractClient::new(&env, &contract_id);
