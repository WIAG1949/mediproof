...
#[contract]
pub struct RecallManagerContract;

#[contractimpl]
impl RecallManagerContract {
...
mod test {
    ...
    fn test_initialize() {
        let env = Env::default();
        env.mock_all_auths();
        let contract_id = env.register_contract(None, RecallManagerContract);
        let client = RecallManagerContractClient::new(&env, &contract_id);
