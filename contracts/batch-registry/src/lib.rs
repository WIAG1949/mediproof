...
#[contract]
pub struct BatchRegistryContract;

#[contractimpl]
impl BatchRegistryContract {
...
mod test {
    ...
    fn test_initialize() {
        let env = Env::default();
        env.mock_all_auths();
        let contract_id = env.register_contract(None, BatchRegistryContract);
        let client = BatchRegistryContractClient::new(&env, &contract_id);
